          ...
          <div>
            ...
            <div class="flex gap-1 md:gap-2">
              <svg
                ...
                class="w-8 h-8 md:w-10 md:h-10 cursor-pointer transition-colors"
                :class="
                  star <= overallRating
              ...
              >
                ...
                <div class="flex gap-2 md:gap-3">
                  <label
                    v-for="score in 5"
                    :key="score"
                    class="cursor-pointer flex items-center gap-1.5 md:gap-2"
                  >
                    <input
                      type="radio"
                      :value="score"
...
const overallRating = computed(() => {
  if (editForm.value.details.length === 0) return 0;
  const sum = editForm.value.details.reduce((acc, detail) => acc + detail.score, 0);
  const average = sum / editForm.value.details.length;
  return Math.floor(average); // Bulatkan ke bawah
});

const setOverallRating = (star) => {
  editForm.value.details.forEach((d) => (d.score = star));
};
