<template>
  <div class="header__signup flex justify-evenly items-center text-right col-8 col-sm-4 fw-semibold relative">
    ...
    <div v-if="authStore.isAuthenticated" class="profile-info flex items-center space-x-2">
      <img
        v-if="authStore.user && authStore.user.image"
        :src="authStore.user.image || previewImage"
        alt="Profile Picture"
      ...
      <div class="relative">
        <button @click="toggleDropdown" class="flex items-center space-x-2 px-3 py-2 rounded-lg bg-white shadow-sm hover:bg-gray-100">
          <span class="text-gray-800 font-semibold">{{ authStore.user ? authStore.user.username : 'User' }}</span>
...
const logout = async () => {
  try {
    await authStore.logout(); // Gantikan dengan aksi logout di store Pinia
...
// Tidak perlu computed userData lagi, gunakan authStore.user langsung
